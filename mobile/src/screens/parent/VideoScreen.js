...
export function VideoScreen() {
  ...
  const load = useCallback(async () => {
    try {
      setError(null);
      const res = await api.get('/resources', { params: { type: 'video' } });
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <ScreenHeader title="Video" showBack />
      <FlatList
        data={items}
        ...
        ListEmptyComponent={
          error ? (
            ...
            <EmptyState emoji="🎬" title="Videolar yo'q" description="O'qituvchi hali video qo'shmagan" />
          )
        }
        renderItem={({ item }) => (
          <Pressable
          ...
          >
            <View style={styles.iconWrap}>
              <Ionicons name="play-circle" size={26} color="#2E3A59" />
            </View>
