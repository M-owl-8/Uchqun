...
| CP-024 | Manager ИРР surface (раҳбар = admin/director role — NOT a new role) | ⬜ PLANNED-NOT-BUILT. **раҳбар = the admin (director) role; reception serves as manager-tier.** Backend already gates manager-sign with `role !== 'teacher'` — admin/reception can sign. Backend has: goal-period `managerSignedAt/By` written by `signGoalPeriod` controller when `role !== 'teacher'`; quarterly facility-level monitoring (OQ-3, ~55 items, manager-only per IRR-SPECIFICATION.md Part A-6c) has no backend or UI. | **Admin portal (Loop 3 re-open or dedicated admin sprint)** — раҳбар is the admin/director role. No new role needed. | Two admin-portal surfaces to build: (1) Goal-period manager signature — admin views teacher-created ИРР goal periods and signs (POST to sign endpoint as admin role → sets managerSignedAt/By). (2) Quarterly facility-level monitoring journal — ~55 items, facility-wide (not per-child), admin-authored. Backend gating is already in place for both. |
| CP-025 | Parent ИРР view (read-only) | ✅ **BUILT — Phase 3e** (`teacher/src/parent/pages/ChildIRR.jsx`, commit pending). 3 parent read endpoints wired: GET /parent/children/:childId/irr, GET /parent/children/:childId/irr/assessment, GET /parent/children/:childId/irr/goals. AGGREGATE only (OQ-4). No per-criterion, no journals, no write path. | Parent portal ✅ | View-only ИРР page in parent portal: progression chart (totalScore/maxPossibleScore per session, progress bars, trend arrows), long-term goals, periods + STGs, quarterly review with parentRecommendations prominently surfaced. Gentle 404 empty state. Nav entry in Sidebar + BottomNav. |
