...
const mockUser = { firstName: 'Ali', lastName: 'Valiyev', avatar: null };
...
function stubLoad(api, messages = []) {
  api.get.mockImplementation((url) => {
    ...
    if (url === '/teacher/messages') return Promise.resolve({ data: { data: messages } });
    return Promise.reject(new Error('Unexpected URL: ' + url));
  });
}

describe('CL-014b Settings (teacher)', () => {
  beforeEach(() => {
  ...
  it('shows my messages button when messages exist and opens history modal', async () => {
    ...
    expect(screen.getByText("O'qituvchi muammo")).toBeTruthy();
  });

  it('calls POST /teacher/message-to-government when message is sent', async () => {
    const api = (await import('../../shared/services/api')).default;
    stubLoad(api);
    ...
    await waitFor(() => {
      expect(api.post).toHaveBeenCalledWith('/teacher/message-to-government', {
