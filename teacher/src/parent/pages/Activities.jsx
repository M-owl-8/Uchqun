...
const Activities = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto pb-20 animate-in fade-in duration-500">
      {/* Header */}
      <Card className="bg-gradient-to-r from-brand-500 to-brand-400 rounded-2xl p-6 md:p-8 shadow-xl border-0 mb-8">
      ...
      {activities.length > 0 ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          {activities.map((activity) => {
            return (
              <div
                key={activity.id}
                className="bg-surface rounded-2xl shadow-lg border border-slate-200 hover:shadow-2xl transition-all duration-300 overflow-hidden group"
              >
                {/* Card Header */}
                <div className="bg-gradient-to-r from-brand-500 to-brand-400 p-5">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      {activity.goal && (
                        <p className="text-sm text-brand-50 line-clamp-2">
                ...
                <div className="p-5 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {activity.startDate && (
                      <div className="flex items-center gap-2 p-3 bg-brand-50 rounded-xl border border-brand-100">
                        ...
                        <div className="min-w-0">
                          <p className="text-xs text-brand-600 font-semibold mb-0.5">{t('activities.startDate') || 'Boshlanish'}</p>
                    ...
                    {activity.endDate && (
                      <div className="flex items-center gap-2 p-3 bg-brand-50 rounded-xl border border-brand-100">
                        <Calendar className="w-4 h-4 text-brand-600 flex-shrink-0" />
                        <div className="min-w-0">
                          <p className="text-xs text-brand-600 font-semibold mb-0.5">{t('activities.endDate') || 'Tugash'}</p>
                          <p className="text-sm font-bold text-slate-900 truncate">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {activity.services.slice(0, 3).map((service, idx) => (
                          <span
                            key={idx}
                            className="px-2.5 py-1 bg-gradient-to-r from-brand-100 to-brand-50 text-brand-700 rounded-lg text-xs font-semibold border border-brand-200/50"
                          >
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 p-3 bg-gradient-to-r from-brand-500 to-brand-400 hover:from-brand-600 hover:to-brand-500 rounded-xl text-white transition-all duration-200 font-semibold text-sm shadow-md hover:shadow-lg"
                  >
        ...
        <Card className="text-center py-20 bg-surface/95 backdrop-blur-sm">
            ...
            <div className="p-6 space-y-6">
              ...
              {selectedActivity.goal && (
                <div className="bg-gradient-to-br from-brand-50 to-brand-100/50 rounded-xl p-5 border border-brand-200">
                  <p className="text-sm font-bold text-brand-700 mb-2">{t('activities.goal') || 'Maqsad'}</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {selectedActivity.startDate && (
                  <div className="flex items-center gap-3 p-4 bg-brand-50 rounded-xl border border-brand-100">
                    ...
                    <div>
                      <p className="text-xs text-brand-600 font-semibold mb-1">{t('activities.startDate') || 'Boshlanish'}</p>
                ...
                {selectedActivity.endDate && (
                  <div className="flex items-center gap-3 p-4 bg-brand-50 rounded-xl border border-brand-100">
                    <Calendar className="w-5 h-5 text-brand-600 flex-shrink-0" />
                    <div>
                      <p className="text-xs text-brand-600 font-semibold mb-1">{t('activities.endDate') || 'Tugash'}</p>
                      <p className="text-sm font-bold text-slate-900">
