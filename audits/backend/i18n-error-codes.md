...
| `ATTENDANCE_PARENT_ONLY` | 403 | Defense-in-depth role check inside `getMyChildAttendance` (PP-ATTENDANCE-SURFACE) — fires when `req.user.role !== 'parent'` despite the `requireParent` middleware (controller-level redundancy per CLAUDE.md "Defense-in-depth role checks"). | "Only parents can view their child's attendance." |
| `ATTENDANCE_CHILD_NOT_ACCESSIBLE` | 403 | Parent passed `?childId=X` where X is not in their own `Child.findAll({ where: { parentId: req.user.id } })` set. Privacy boundary on a minor's record — never leak attendance for someone else's child. | "This child is not yours — attendance access denied." |
| `ATTENDANCE_FETCH_FAILED` | 500 | Unexpected server error inside `getMyChildAttendance` (DB read failure, Sequelize crash). | "Failed to load attendance. Please try again." |
