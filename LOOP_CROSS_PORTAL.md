...
| CP-024 | Manager ИРР surface (раҳбар = admin/director role — NOT a new role) | ✅ **BUILT — CP-024** (`admin/src/pages/ManagerIRR.jsx`, commit 79cc9bf). Two admin surfaces: (1) Goal-period signature — lists school children, lazy-loads IRR+periods, sign button → POST /teacher/goal-periods/:id/sign as admin → managerSignedAt/By. (2) Quarterly facility-level monitoring form + history list → GET/POST /admin/irr/quarterly-entries. Nav entry added to Sidebar. 6/6 tests green. | Admin portal ✅ | Admin (раҳбар) signs teacher-created ИРР goal periods and authors quarterly facility-wide monitoring journal. |
