  ...
  return (
    <div className="min-h-screen bg-cream flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="bg-surface rounded-xl border border-warm-200 shadow-sm p-8">
          ...
          <h1 className="text-xl font-semibold text-warm-900 text-center mb-1">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {[
              ...
              <div key={key}>
                <label className="block text-xs font-medium text-warm-800 mb-1">{label}</label>
                <div className="relative">
                  <input
                    ...
                    required
                    className="w-full h-10 px-3.5 pr-10 text-sm bg-surface border border-warm-300 rounded-md text-warm-900 focus:outline-none focus:ring-2 focus:ring-brand-600/30 focus:border-brand-600"
                    autoComplete={key === 'currentPassword' ? 'current-password' : 'new-password'}
                  />
                  <button
                    type="button"
                    onClick={() => toggle(field)}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-warm-500 hover:text-warm-800 p-1"
                    tabIndex={-1}
            ...
            <button
              type="submit"
              disabled={saving}
              className="w-full inline-flex items-center justify-center h-11 px-4 text-sm font-medium bg-brand-600 hover:bg-brand-700 disabled:opacity-50 text-walnut-text rounded-md shadow-xs transition-colors"
            >
              {saving
