...
>
> 1. **No data was lost in the pause — one row is Playwright test residue.**
>    *(Corrected: an initial reading of this evidence concluded the pre-pause blobs had died. That was wrong.)*
>    Fetching `032cc714…` returns `HTTP 200` with a 70-byte 1×1 PNG. That is not damage — it is exactly what
>    was uploaded. The row's title is `S22V3-1780984076667`, generated by
>    `tests/s22v3-blocked-rows.spec.js:313` (`S22V3-${Date.now()}`), which uploads `FIXTURE.tinyPng`, the
>    1×1 white PNG defined at that spec's line 27. Its base64 differs from the controller's own sentinel,
>    which is why the md5 did not match (`09026025…` vs `f829b914…`) — the payload came from Appwrite, not
>    from an error path. The other 2026-06-09 upload, `484a4fe7…`, returns a genuine **159,504-byte
>    photograph**. **Appwrite retained everything through the pause.** Only the test row should be retired,
>    and only because test data escaped into production: it is attached to a real child and still carries an
>    unread parent notification for a blank image. Migration `20260915000001-retire-test-residue-media.js`.
...
>    fixed in commit `349c847c`.
>
> **Independent corroboration:** between 19:50 and 19:53 UTC on 2026-09-14, `tarbiyachi1@tmm3.uz` uploaded
> **six real photographs** to production through the normal UI ("Dars jarayonlari", "Tushlik",
> "LFK mashg'ulotlari", "Ginrovanna terapiyasi"), all stored in Appwrite. Live media is now 12 rows.
>
...
2b. ~~**NEW — delete the 2 dead media rows.**~~ **Superseded.** Nothing died: `484a4fe7…` is a healthy 159 KB photo and `032cc714…` is Playwright residue (`s22v3-blocked-rows.spec.js`) that always contained a 1×1 pixel. Only the test row is retired, by migration `20260915000001-retire-test-residue-media.js`. **Open question instead: why does a Playwright spec write to the production database?** Test runs leaving real rows attached to real children — with parent notifications — is a process defect worth closing at the source.
...
8. **The 12 orphaned `documents` rows — DO NOT bulk-delete. Needs a decision.**
   Investigated 2026-09-15 and deliberately left untouched. Facts:
   - No backend code produces the `/uploads/documents/<uuid>.pdf` path (`config/storage.js:191` writes to `/uploads/media/`), and the files **404 in production**. They reference blobs that never existed in a deployed container.
   - They belong to **12 distinct reception accounts** on the Uzbek domains (`qabul@tmm3.uz`, `qabul2@smm4.uz`, …), not to the `reception1-4@uchqun.uz` seeds: 6 `approved`, 5 `pending`, 1 `rejected`.
   - **Deleting them is not inert.** `adminReceptionController.js:250-258` recomputes activation from the table on every approval: `allDocuments.every(d => d.status === 'approved') && allDocuments.length > 0`. Removing a `pending` or `rejected` row can therefore flip a later approval into **premature account activation** (`documentsApproved = true`, `isActive = true`, `isVerified = true` at `:261-263`). Login access today is unaffected — `documentsApproved` is a stored column on `users`, read at `middleware/auth.js:108` — but the future path is a real hazard.
   - Deleting the 6 `approved` rows would also erase the record that an admin approved a document that could never have been viewed. That is the finding; deleting it destroys the evidence rather than resolving it.

   Options for the owner: (a) leave as-is and require re-upload before real-user launch; (b) reset the affected receptions to `pending` and require re-upload, which is the honest state; (c) delete only after `approveDocument`'s recomputation is made resilient to missing rows. **Recommend (b), but it changes account state for 12 users and is not mine to choose.**
