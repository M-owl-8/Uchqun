...
| CP-024 | Manager ИРР surface (раҳбар role) | ⬜ PLANNED-NOT-BUILT. Backend has: goal-period `managerSignedAt/By` written by `signGoalPeriod` controller when `role !== 'teacher'`; quarterly facility-level monitoring (OQ-3, ~55 items, manager-only per IRR-SPECIFICATION.md Part A-6c) has no backend or UI. | Manager/раҳбар portal — open question: is this the Admin portal under a manager sub-role, or a dedicated раҳбар account? Must be resolved before building. | Two manager surfaces to build: (1) Goal-period manager signature — manager views teacher-created ИРР goal periods and signs (POST /teacher/goal-periods/:id/sign as manager role → sets managerSignedAt/By). (2) Quarterly facility-level monitoring journal — ~55 items, facility-wide (not per-child), manager-authored. Neither is in the teacher portal. Resolve portal/role ownership before building. |
| CP-025 | Parent ИРР view (read-only) | ⬜ PLANNED-NOT-BUILT. Backend read endpoints exist from Phase 2 (GET /parent/children/:id/irr, GET /parent/irr/:irrId/…). No parent UI built. | Parent portal — next phase after teacher-side ИРР is complete. | Parent view should show: ИРР header, assessment progression, long-term goals, goal periods + STGs, quarterly review fields, teacher/manager signatures. Read-only — no parent editing. Build as a dedicated parent ИРР page after Teacher Loop S5 is fully closed. |
