...
**Numbering:** ids are permanent and never reused. New defects continue from D-73.
...
| D-70 | degrades-trust | **FIXED** `b768b0a1` | 25 `[REVERT-TEST: BUG]` tests asserted a bug against a LOCAL REPLICA of buggy code — proven to pass while production was broken | `deep3/P4-SUITE-INTEGRITY.md` §6 |
| D-71 | degrades-trust | **OPEN** | the Playwright suite — 24 files, 372 tests — is in no workflow and last ran 2026-06-10; its result cannot affect anything | `deep3/P4-SUITE-INTEGRITY.md` §5 |
| D-72 | degrades-use | **OPEN** | the admin suite fails intermittently under parallel load; mitigated with a 30s timeout, undiagnosed | `deep3/P4-SUITE-INTEGRITY.md` §2 |
