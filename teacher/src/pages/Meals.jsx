...
const Meals = () => {
  ...
  // Meal-type colors confirmed as-is (S29/Q18): Lunch/Dinner stay blue-* as semantic meal colors.
  const mealConfigs = {
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500">
        ...
        <div className="flex items-end gap-3">
          {isTeacher && (
            <button
              ...
              className="flex items-center gap-2 px-5 py-3 bg-brand-600 text-white rounded-xl font-bold hover:bg-brand-700 transition-colors shadow-sm"
            >
          ...
          <div className="relative">
            ...
            <select
              ...
              className="appearance-none bg-surface border border-slate-200 text-slate-900 font-bold rounded-xl px-6 py-3 pr-12 focus:ring-2 focus:ring-brand-500 shadow-sm cursor-pointer"
            >
              {dates.map((date) => (
      ...
      <div className="grid grid-cols-1 gap-6">
        {filteredMeals.length > 0 ? (
                  ...
                  <div className="flex-1 space-y-3">
                    ...
                    <div className="flex items-center gap-6 pt-2">
                      <div className="text-xs font-bold text-slate-400 uppercase tracking-tighter flex items-center gap-1.5">
                        {t('mealsPage.quantity')}: <span className="text-slate-900">{meal.quantity}</span>
                      </div>
                    ...
                    {meal.specialNotes && (
                      <div className="flex items-start gap-2 p-4 bg-slate-50 rounded-2xl border border-slate-100 mt-4 group-hover:bg-surface transition-colors">
                        ...
                        <p className="text-xs text-slate-500 leading-relaxed font-medium">
                          <span className="text-slate-900 font-bold">Eslatma:</span> {meal.specialNotes}
                        </p>
                      </div>
                    )}
                  </div>
          ...
          <div className="text-center py-24 bg-surface rounded-[3rem] border-2 border-dashed border-slate-200">
            <Utensils className="w-16 h-16 text-slate-200 mx-auto mb-4" />
            <p className="text-slate-400 font-bold text-lg">{t('mealsPage.empty')}</p>
          </div>
        )}
      </div>
          ...
          <div className="relative z-10 space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center md:text-left">
              ...
              <div className="space-y-1">
                <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">{t('mealsPage.quality')}</p>
          ...
          <div className="bg-surface rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <label htmlFor="eaten" className="text-sm font-medium text-slate-700">
