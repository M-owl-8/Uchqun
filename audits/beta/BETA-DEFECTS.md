...
- **S22-V4 update (2026-06-11):** Backend theory **refuted** — 36/36 parent API logins hard-pass across three sweeps (12 accounts × 3 runs, HTTP 200 + `/auth/me` role=parent, `tests/s22v4-probe.spec.js`). The defect is a **client-side race** and it reproduces reliably: parent3's UI form login succeeded (login 200, navigated to `/`) but the app bounced back to `/login` within ~1–5 s in **2 of 3** attempts (evidence: `screens/S22V4-P-001-def009-bounce-to-login.png`). Debug trace shows the login page fires `/auth/me` → 401 → interceptor `/auth/refresh` → 401 **before** login; when that chain resolves after the successful login navigation, the unauthenticated handler clears auth and redirects — explaining the intermittent "stays at /login" symptom across different parent accounts (originally parent3/6/7/10/12). Suspected layer revised: **Frontend** — `shared/services/api.js` onUnauthenticated handler racing a concurrent successful login. Severity stays **P1**; matrix P-001 set to FAIL.
- **S23 fix (2026-06-11):** Two racers, both epoch-guarded:
  1. **Interceptor** (`shared/services/api.js`, commit `4df1cf67`): the pre-login bootstrap `/auth/me` 401 kicks off a refresh that 401s (no cookie); its failure handler called `clearAuth()` unconditionally. `createApi` now tracks `authEpoch`, bumped on every successful `/auth/login` response; both `clearAuth()` call sites only fire if the epoch is unchanged since the 401 arrived. A 401 from the pre-login world can no longer clear a post-login session.
  2. **AuthProvider bootstrap** (`shared/context/createAuthContext.jsx`, commit `d472766f`): the bootstrap's own `catch` ran `localStorage.removeItem + setUser(null)` when the `/auth/me` rejection propagated — wiping a login that completed inside the refresh round-trip window. Found because the streak proof after fix #1 alone was **19/20** (one bounce at attempt 14). `AuthProvider` now tracks `loginEpochRef`, bumped on successful `login()`; the bootstrap skips all state writes (success and catch paths) if a login happened after it started.
- **Repeatability proof (`tests/s23-def009-proof.spec.js`, production, post-deploy `index-DZOu5mQ7.js`):** **20/20 consecutive cold UI logins** (fresh browser context each, 390px, rotating parent3/6/7/10/12/1 — the originally-affected accounts) landed on the dashboard and stayed through a 5 s stability window. Zero bounces. Pre-fix rate was 2-of-3; fix #1 alone 19/20; both fixes 20/20.
- **Regression:** fully-invalid session (both cookies cleared) still redirects to `/login` — the legitimate 401 → clearAuth path asserted intact. All four portals (teacher, admin, reception, government) share this `createApi`/`createAuthContext` and all four UI logins verified clean post-fix (each held a 5 s stability window).
- **Status:** ✅ FIXED (S23, 2026-06-11) — commits `4df1cf67` + `d472766f`, deployed to Railway. Matrix P-001 → PASS.
...
- **S25 fix (2026-06-11, commit `dff64571`):** fail closed. The verify step found the defect was **broader than logged**: besides the unvalidated `childId`, a NULL-schoolId admin with NO `childId` fell through to an **unscoped platform-wide** `getMedia` query, and `getMediaItem` could fetch **any media item by id** (its admin branch only scoped when `schoolId` was set). All three paths now fail closed: `getMedia` → 403 on any `childId`, empty list otherwise (no query executed); `getMediaItem` → 404 (no query executed). With-schoolId admin, teacher, parent, and government branches untouched.
- **Provisioning re-check (verify step 3):** no non-DB path can create a NULL-schoolId admin — `resolveEmailDomain(creator,'admin',{schoolId})` throws `ACCOUNT_CREATE_FORBIDDEN_HIERARCHY` without a schoolId, and both creation paths (`createAdmin` POST /government/admins, registration approval) route through it and 403. `createAdmin`'s `...(schoolId && { schoolId })` spread never executes with schoolId absent. Severity stays P2-latent; no new finding.
- **NULL-schoolId simulation:** mocked `req` objects in `backend/__tests__/media.test.js` (no such account created anywhere) — 3 new tests assert 403-on-childId + empty-list (getMedia) and 404 (getMediaItem), each also asserting `Media.findAll`/`findOne` was never called. 16/16 suite pass.
