...
export function TavsiyaScreen() {
  ...
  const load = useCallback(async () => {
    try {
      setError(null);
      const res = await api.get('/resources', { params: { type: 'recommendation' } });
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <ScreenHeader title="Tavsiya" showBack />
      <FlatList
        data={items}
        ...
        ListEmptyComponent={
          error ? (
            ...
            <EmptyState emoji="💡" title="Tavsiyalar yo'q" description="O'qituvchi hali tavsiya qo'shmagan" />
          )
        }
        renderItem={({ item }) => (
          <Pressable
          ...
          >
            <View style={styles.iconWrap}>
              <Ionicons name="bulb" size={26} color="#2E3A59" />
            </View>
