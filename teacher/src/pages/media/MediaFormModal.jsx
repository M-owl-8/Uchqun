    ...
    <div className="fixed inset-0 z-[101] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-surface rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={onSubmit} className="p-6 space-y-4">
          {isTeacher && (
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">
                {t('mediaPage.modal.child')} <span className="text-error-500">*</span>
              </label>
              {childList.length === 0 ? (
                <div className="p-4 bg-warning-50 border border-yellow-200 rounded-lg">
                  <p className="text-sm text-warning-800">{t('mediaPage.modal.childHelp')}</p>
                </div>
              ) : (
                <select
                  required
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">
              {t('mediaPage.modal.title')} <span className="text-error-500">*</span>
            </label>
            <input
              type="text"
              required
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">
                {t('mediaPage.modal.date')} <span className="text-error-500">*</span>
              </label>
              <input
                type="date"
                required
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">
                {t('mediaPage.modal.type')} <span className="text-error-500">*</span>
              </label>
              <select
                value={formData.type}
          ...
          {!editingMedia && (
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">
                {t('mediaPage.modal.file')} <span className="text-error-500">*</span>
              </label>
              <input
                type="file"
                accept="image/*,video/*"
                ...
                className="w-full px-4 py-3 border border-slate-200 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-transparent"
              />
              <p className="text-xs text-slate-500 mt-1">{t('mediaPage.modal.fileHelp')}</p>
            </div>
          )}
