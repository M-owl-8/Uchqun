...
describe('superAdminController', () => {
  ...
  describe('sendMessage', () => {
    ...
    it('400 when only whitespace', async () => {
      const req = { user: { id: 'u1', role: 'parent' }, body: { subject: '  ', message: '  ' } };
      const res = mkRes();
      await sendMessage(req, res);
      expect(res.status).toHaveBeenCalledWith(400);
    ...
    it('creates message and trims fields', async () => {
    ...
    it('500 with hint when table missing', async () => {
      mockCreate.mockRejectedValue(new Error('relation "super_admin_messages" does not exist'));
  ...
  describe('getMessages', () => {
    it('returns messages and pagination', async () => {
    ...
    it('filters by isRead query', async () => {
  ...
  describe('getMessageById', () => {
    ...
    it('marks unread message as read on read', async () => {
  ...
  describe('replyToMessage', () => {
    ...
    it('persists reply and repliedAt', async () => {
      const save = jest.fn().mockResolvedValue();
  ...
  describe('markMessageRead', () => {
    it('clears readAt when isRead=false', async () => {
      const save = jest.fn().mockResolvedValue();
      const m = { id: 'm1', save, toJSON: () => ({}) };
      mockFindByPk.mockResolvedValue(m);
