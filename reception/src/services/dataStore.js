...
// Full-featured Kindergarten Management System
...
export const dataStore = {
  ...
  createNews: (newsData) => {
    ...
    const newNews = {
      ...
      id: generateId(),
      createdAt: new Date().toISOString(),
      publishedAt: newsData.published ? new Date().toISOString() : null,
    };
    news.push(newNews);
  ...
  updateNews: (id, newsData) => {
    ...
    if (index !== -1) {
      ...
      news[index] = {
        ...
        publishedAt: newsData.published && !existing.publishedAt ? new Date().toISOString() : existing.publishedAt,
      };
      localStorage.setItem(STORAGE_KEYS.NEWS, JSON.stringify(news));
      return news[index];
    }
